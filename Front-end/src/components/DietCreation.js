...
const DietCreation = (props) => {
    ...
    useEffect(()=>{
        ...
        if(currentUser) {
            UserService.getDiet(currentUser.email,props.date).then(
                (response) =>{
            ...
            { rows!==null && rows.map(obj => (
                <Grid container spacing={2} className="diet-creation-container" key={obj.id}>
                    ...
                    <Grid item lg={9} xl={9} md={7} sm={6} xs={5}>
                        <TextField  variant="outlined"  required className="components" value={obj.diet} onChange={(e) => changeDiet(obj.id, e.target.value)}></TextField>
                    </Grid>
